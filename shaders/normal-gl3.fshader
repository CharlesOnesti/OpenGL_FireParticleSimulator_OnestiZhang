...
void main() {
  ...
  vec3 normal = normalize(vNTMat * (texture(uTexNormal, vTexCoord).xyz * 2 + vec3(-1.0, -1.0, -1.0))); // x * 2 - 1
  ...
  vec3 color = texture(uTexColor, vTexCoord).xyz * (diffuse * vec3(1, 0.6, 0.6)) + specular * vec3(1, 0.6, 0.6);
