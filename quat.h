...
inline Quat power(const Quat& a, const double t) {
  float beta = norm(Cvec3(a(1), a(2), a(3)));
  if (abs(beta) < 0.0001) {
    ///todo - check
    return Quat(1, 0, 0, 0);
  ...
  Cvec3 k = Cvec3(a(1), a(2), a(3)) / beta;

  //why the fuck is this line red, the arguments types are correct
  float angle = atan2(beta, a(0));

  //same here
  return Quat(cos(t * angle), k * sin(t * angle));

}

inline Quat slerp(const Quat& a, const Quat& b, const double t){
